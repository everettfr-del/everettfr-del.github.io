...
You'll require the following list of software to build this repository. Each will be listed along with instructions on how to install it. Most steps will require the use of your command console - open it by typine 'cmd' into your Windows search bar, pressing Cmd+space on mac, or by pressing Ctrl+Alt+T on Linux.
...
If you do not have Quarto installed, download and run the Windows installer from the official Quarto website:
...
q()
```
to exit the R environment. Lastly, you'll need to type the following to render the site:
```bash
quarto render
```
...
## Data sources
The data source for my two blog posts regarding Gapminder data processing is [Gapminder](https://www.gapminder.org/data/) data set, compiled by the Gapminder Organization in Sweden to show real-world development trends over time and fight misconceptions about the state of the world through data. It contains infromation regarding life expectancy, population, and GDP per capita for 140+ nations around the world, assembled from a variety of sources such as the Institute for Health Metrics and Evaluation, the US Census Bureau’s International Database, the United Nations Statistics Division, and the World Bank.

An internet connection is required during initial environment setup to obtain Python and R dependencies as well as do any necessary software installion. Once those dependencies are installed, the site build itself does not require network access to retrieve the Gapminder dataset as it is already installed withinin the packages.